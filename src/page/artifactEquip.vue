<template>
    <div class="set_container">
        ...
        <div class="user_container">
            <!-- 角色筛选器 -->
            <div class="filterBox">
                <button type="button" class="btn btn-genshin dropdown-genshin-toggle" data-bs-toggle="dropdown"
                    aria-expanded="false">
                    {{ filterShow }}
                </button>
                <ul class="dropdown-menu dropdown-menu-lg-end">
                    <li @click="filterCharacter='default'">
                        <a class="dropdown-item"
                            :class="{itemSelected:filterCharacter==='default'}">{{ $t('msg.default') }}</a>
                    </li>
                    <!-- 属性筛选 -->
                    ...
                    <li v-for="element in $artiConst.val.elementType" :key="element" @click="filterCharacter=element">
                        <a class="dropdown-item"
                            :class="{itemSelected:filterCharacter===element}">{{ $t('element.'+element) }}</a>
                    </li>
                    <!-- 武器筛选 -->
                    ...
                    <li v-for="weapon in $artiConst.val.weaponType" :key="weapon" @click="filterCharacter=weapon">
                        <a class="dropdown-item"
                            :class="{itemSelected:filterCharacter===weapon}">{{ $t('weaponType.'+weapon) }}</a>
                    </li>
                    <!-- 地区筛选 -->
                    <li class="filterLable">{{ $t('nation.nation') }}</li>
                    <li v-for="nation in $artiConst.val.nation" :key="nation" @click="filterCharacter=nation">
                        <a class="dropdown-item"
                            :class="{itemSelected:filterCharacter===nation}">{{ $t('nation.'+nation) }}</a>
                    </li>
                </ul>
            </div>
            <!-- 角色列表 -->
            <div class="characterBox" ref="characterBox">
                <div class="characterShow" v-for="character in characterList" :key="character"
                    :class="{isSelect:selectCharacter===character.name}" @click="selectCharacter=character.name"
            ...
            <!-- 按钮 -->
            <div class="buttonBox">
                <button class="btn btn-genshin btn-switch" :class="{disable:selectCharacter===''}"
                    :disabled="(selectCharacter==='')" @click="chengeArtifact()" v-show="!isSame">
        ...
        <div class="bg-container">
            ...
            <div class="artifactDetailBox">
                ...
                <artifact-show :index="index" :showdetail="artifact" :language="language" :showButton="false"
                    @lock="lockChange" v-if="JSON.stringify(artifact)!=='{}'"
                ...
                <div class="comparedBox" v-show="!(this.oldArtifact === undefined) && !isSame">
                    ...
                    <div class="newName"><img src="../assets/images/Arrow.png" alt="Arrow" draggable="false">
                        {{ artifactName }}</div>
                    ...
                    <div class="setChangeInfo">
                        ...
                        <span v-for="(value,key) in setBonusChange" :key="key"
                            :class="value[1]?'getBonus':'lostBonus'">
                    ...
                    <div class="attrChangeList">
                        <div class="attrChangeInfo" v-for="(value,key) in changeList" :key="key">
                            ...
                            <span class="valueChange"
                                :class="value>0?'value_up':'value_down'">{{ formatValue(key,value) }}</span>
                        </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            // 当前圣遗物信息
            this.artifact = this.$artifact.getArtifact(this.$artifact.AUSList[this.index].symbol, JSON.parse(
                localStorage.userSetting).language);
            if (this.language === "zh") {
        ...
        watch: {
            ...
            selectCharacter(val) {
                ...
                // 更新已装备的圣遗物
                let oldIndex = this.$artifact.getIndex(this.$artifact.setList[this.setIndex][this.$artifact.AUSList[
                    this.index].part]);
            ...
            oldArtifact(val) {
                ...
                this.setBonusChange = {};
                let that = this,
                    artifactNow = this.$artifact.AUSList[this.index],
                    ...
                    compareFunc = function (arr, type) {
                        for (let i in arr) {
                            ...
                            if (Object.prototype.hasOwnProperty.call(that.changeList, entry)) {
                                type === "new" ? that.changeList[entry] += entryValue :
                                    that.changeList[entry] -= entryValue;
                            } else {
                                type === "new" ? that.changeList[entry] = entryValue :
                                    that.changeList[entry] = -entryValue;
                            }
        ...
        methods: {
            ...
            avatarSrc(name) {
                try {
                    let src = require("../assets/images/character/Character_" + name.replace(/\s+/g, '_') +
                        "_Thumb.png");
                    return src;
                } catch (error) {
                    return "";
            ...
            chengeArtifact() {
                ...
                this.artifact = this.$artifact.getArtifact(this.$artifact.AUSList[this.index].symbol, this.language);
                // isSame判定
                this.isSame = true;
            },
            // 圣遗物锁定
            lockChange(index) {
                this.$artifact.lock(Number.parseInt(index));
                // 更新artifact
                this.artifact = this.$artifact.getArtifact(this.$artifact.AUSList[this.index].symbol, this.language);
            },
            // 卸下圣遗物
            removeArtifact() {
                let part = this.$artifact.AUSList[this.index].part;
    ...
    .characterBox {
        ...
        .characterShow {
            ...
            .avatarBox {
                width: 4rem;
                height: 4rem;
                border-radius: .25rem .25rem 1rem 0;
                overflow: hidden;
        ...
        .artifactDetailBox {
            ...
            .comparedBox {
                ...
                .setChangeInfo {
                    font-size: 1rem;
                    color: $genshin_white;
                    border-top: solid .125rem transparentize($genshin_green, 0.8);
                    border-bottom: solid .125rem transparentize($genshin_green, 0.8);
                    padding: .125rem .25rem;
